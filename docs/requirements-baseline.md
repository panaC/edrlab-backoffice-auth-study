...
| ID | Priority | Requirement | Source |
| --- | --- | --- | --- |
| `ARCH-001` | Must | The selected study architecture is a Backoffice BFF, a central IAM Control Plane, and one or more backend API resource servers. | [README](../README.md), [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
| `ARCH-002` | Must | The IAM Control Plane is the source of truth for members, credentials or external identities, clients, roles, permissions, role assignments, token contracts, key metadata, and IAM audit logs. | [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
| `ARCH-003` | Must | The Backoffice BFF owns browser sessions and OIDC callback handling, but it must not become a second IAM authority or permanent role database. | [Minimal architecture](./minimal-backoffice-iam-architecture.md), [BFF sessions](./bff-sessions-and-token-handling.md) |
| `ARCH-004` | Must | Backend APIs own business logic and data, validate access tokens, and enforce operation-level permissions server-side. | [README](../README.md), [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
| `ARCH-005` | Must | Backend services must not read the IAM database directly for authorization. They should use token validation, introspection, or a documented authorization/check path. | [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
| `ARCH-006` | Should | The first minimal PoC may use one demonstration backend API resource server to prove token validation and permission enforcement. | [README](../README.md), [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
| `ARCH-007` | Future | Service clients and workers should remain distinct from human users if service-to-service authentication becomes in scope later. | [Minimal architecture](./minimal-backoffice-iam-architecture.md), [Initial permission model](./initial-permission-model.md) |
...
| ID | Priority | Requirement | Source |
| --- | --- | --- | --- |
| `MEM-001` | Must | Administrators can create, read, update, list, disable, delete or retain, restore where policy allows, and remove access for members. | [README](../README.md), [Member lifecycle](./member-lifecycle.md) |
| `MEM-002` | Must | Member records use stable identifiers separate from mutable attributes such as email, username, display name, title, or department. | [Member lifecycle](./member-lifecycle.md) |
| `MEM-003` | Must | Non-active members cannot obtain new effective backoffice access. | [Member lifecycle](./member-lifecycle.md), [Threat model](./threat-model.md) |
| `MEM-004` | Should | The first baseline treats `active` and `disabled` as the required lifecycle states; `invited`, `archived`, `deleted`, and `suspended` remain policy decisions. | [Member lifecycle](./member-lifecycle.md), [Open questions](./open-questions.md) |
| `MEM-005` | Should | Member recovery, authenticator reset, MFA reset, and email changes are sensitive lifecycle operations with explicit authorization and audit behavior. | [Member lifecycle](./member-lifecycle.md), [Administrator authentication policy](./administrator-authentication-policy.md) |
...
| ID | Priority | Requirement | Source |
| --- | --- | --- | --- |
| `API-001` | Must | The IAM Control Plane exposes or supports a REST administration API for members, roles, permissions, role assignments, clients where relevant, service access, and audit-supporting operations. | [README](../README.md), [Minimal architecture](./minimal-backoffice-iam-architecture.md) |
...
| ID | Priority | Requirement | Source |
| --- | --- | --- | --- |
| `SEC-001` | Must | The design does not rely on custom cryptography, unsigned JWTs, skipped issuer validation, skipped audience validation, frontend-only authorization checks, or long-lived access tokens without justification. | [Threat model](./threat-model.md), [Security best practices](./wiki/09-security-best-practices.md) |
| `SEC-002` | Must | Cookie-authenticated BFF routes use CSRF defenses for state-changing requests. | [BFF sessions](./bff-sessions-and-token-handling.md), [Threat model](./threat-model.md) |
| `SEC-003` | Must | OAuth clients use exact redirect URI registration, environment separation, and audited redirect/client changes where applicable. | [Threat model](./threat-model.md), [Initial permission model](./initial-permission-model.md) |
| `SEC-004` | Should | Administrator authentication strength, including MFA, step-up, passwordless options, hardware-backed authenticators, and recovery controls, is evaluated before production. | [Administrator authentication policy](./administrator-authentication-policy.md), [Open questions](./open-questions.md) |
...
| `OPS-004` | Should | Key, client-secret, credential, and backup-key rotation procedures are documented before production. | [Operational model](./operational-model.md), [Threat model](./threat-model.md) |
| `OPS-005` | Should | Monitoring and incident-response playbooks cover token theft, refresh-token theft, client secret leaks, signing-key compromise, compromised administrators, bad role assignments, audience-validation failures, provider outage, and audit failure. | [Operational model](./operational-model.md), [Threat model](./threat-model.md) |
| `OPS-006` | Should | The project explicitly accepts, rejects, or defines break-glass access before production. | [Operational model](./operational-model.md), [Administrator authentication policy](./administrator-authentication-policy.md) |
