...
| `SEC-CLAIM-001` | `Implemented` | Stale token role claims do not keep access after role removal. | Covered by access-stop tests and Docker Keycloak smoke evidence that a real access token minted with a now-stale `access-check-demo:consult` role claim returns confirmed `403 KO` after IAM-managed role removal. |
...
| `SEC-TOKEN-003` | `Implemented` | Expired, invalid-signature, unsupported-algorithm, or wrong-client token is rejected. | Covered by OIDC subject-token and service-token introspection tests for expiry, subject, expected-client, issuer, audience, and inactive or invalid tokens. Invalid signature and unsupported algorithm rejection are delegated to Keycloak introspection because the Phase 6 Docker MVP does not use local JWT validation. |
...
| `SEC-SUBJECT-003` | `Implemented` | Different-subject activation or supported control-plane mutation cannot rebind the link. | Covered by different-subject onboarding conflict, protected-field mutation, and path-target mutation tests; direct Keycloak `iam.linked_subject` mutation is tracked separately as `SEC-DRIFT-004`. |
...
| `SEC-FAIL-004` | `Implemented` | Unknown service/role, inactive account, disabled role, or archived role fails closed. | Covered by `authorization/check` tests for unknown service and role IDs, disabled and archived accounts, disabled and archived service roles, and role removal. |
