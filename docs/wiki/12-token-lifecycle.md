...
For a BFF architecture, refresh tokens should remain server-side. The browser should not store them in `localStorage`, `sessionStorage`, readable cookies, URLs, or frontend logs. See [Web Sessions, Cookies, and BFF Pattern](./24-web-sessions-cookies-and-bff.md) for the browser and session boundary.
